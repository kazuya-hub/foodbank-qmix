...
from .storage import SQLiteWriter, SQLiteStorage
...
class _ArrayRegistry:
    ...
    def _pad_array_to_shape(self, array: np.ndarray, target_shape: tuple[int, ...], name: str) -> np.ndarray:
        """
        Pad array with zeros to match target_shape.
        
        Args:
            array: Input array to pad
            target_shape: Target shape to pad to
            name: Array name for error messages
            
        Returns:
            Zero-padded array matching target_shape
            
        Raises:
            ValueError: If dimension count doesn't match or any dimension exceeds target
        """
        if array.ndim != len(target_shape):
            raise ValueError(
                f"Array '{name}': 次元数が一致しません: expected {len(target_shape)} dimensions, got {array.ndim} dimensions"
            )
        
        for i, (array_dim, target_dim) in enumerate(zip(array.shape, target_shape)):
            if array_dim > target_dim:
                raise ValueError(
                    f"Array '{name}': 次元{i}のサイズが超過しています: expected ≤{target_dim}, got {array_dim}"
                )
        
        # Calculate padding for each dimension (pad at the end)
        pad_width = [(0, target_dim - array_dim) for array_dim, target_dim in zip(array.shape, target_shape)]
    ...
    def log(
        ...
        - shape/dtype MUST match registered schema (or be padded if zero_padding=True)
        - key_values MUST be a dict matching registered keys
        
        Args:
            name: Registered array name
            key_values: Dictionary of key-value pairs matching registered keys
            array: NumPy array to log
            zero_padding: If True, pad array with zeros to match schema shape when dimensions match
                         but sizes differ (all array dimensions must be ≤ schema dimensions)
        """
        ...
        if array.shape != schema.shape:
            if zero_padding:
                # Pad array to match schema shape
                array = self._pad_array_to_shape(array, schema.shape, name)
            else:
                raise ValueError(f"Shape mismatch: expected {schema.shape}, got {array.shape}")
        
        # Check dtype (after potential padding)
...
def log(name: str, key_values: dict[str, Any], array: np.ndarray, zero_padding: bool = False) -> None:
    """
    Log a array.
    ...
    if _logger is None:
        raise RuntimeError("Logger not initialized")
    _logger.log(name, key_values, array, zero_padding=zero_padding)
