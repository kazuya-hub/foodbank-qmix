...
class ArrayReader:
    ...
    def _load_schema(self) -> None:
        ...
        # Parse shape (support both "(1,2,3)" and "torch.Size([1,2,3])" formats)
        shape_str = meta_dict["shape"]
        try:
            shape = ast.literal_eval(shape_str)
        except (ValueError, SyntaxError):
            # Try to extract from torch.Size([...]) format
            match = re.search(r'torch\.Size\(\[([^\]]+)\]\)', shape_str)
            if match:
                shape = ast.literal_eval(f"[{match.group(1)}]")
            else:
                raise ValueError(f"Unable to parse shape: {shape_str}")
